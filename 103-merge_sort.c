...
/**
 * spliting - recursive function to split data into merge tree
 *
 ...
 * Return: No Return
 */
void spliting(int *array, int *buff, int min, int max, int size)
{
	...
	spliting(array, buff, min, max, size);
	...
	print_check(array, minL, maxL);

	printf("[right]: ");

	print_check(array, minR, maxR);
	_merge(array, buff, minL, maxL, minR, maxR);
	copy_data(array, buff, size);

	printf("[Done]: ");
	print_check(array, minL, maxR);
}
/**
...
void merge_sort(int *array, size_t size)
{
	...
	spliting(array, buff, 0, size - 1, size);
