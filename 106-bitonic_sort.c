...
/**
 * _merge - swap the elements to sort
 * @array: Array to sort
 ...
 * Return: Nothing
 */
void _merge(int *array, int low, int size, int dir, const int r_size)
{
	int k = size, i = low;
	...
	if (size > 1)
	{
		...
		_merge(array, low, k, dir, r_size);
		_merge(array, low + k, k, dir, r_size);
	}
}
/**
 * _sorting - segmentate the array
 ...
 * Return: Nothing
 */
void _sorting(int *array, int low, int size, int dir, const int r_size)
{
	int k = size;
	...
	if (size > 1)
	{
		...
		k = size / 2;
		_sorting(array, low, k, 1, r_size);

		_sorting(array, low + k, k, 0, r_size);

		_merge(array, low, size, dir, r_size);
		if (dir == 1)
		{
...
void bitonic_sort(int *array, size_t size)
{
	...
	_sorting(array, 0, (int)size, up, r_size);
}
